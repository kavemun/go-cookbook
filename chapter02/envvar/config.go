...
// LoadConfig will load files optionanlly from the json file stored at path,
// then will override those values based on the envconfig struct tags.
// The envPrefix is how we prefix out environment variables.
func LoadConfig(path, envPrefix string, config interface{}) error {
	...
	return errors.Wrap(err, "error loading config from any")
}
